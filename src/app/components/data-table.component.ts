...
// Calculează offset-ul pentru coloanele fixe la stânga
getLeftOffset(column: ColumnDefinition): number {
    let offset = 0;
    for (let col of this.columns) {
      if (col.fixed === 'left') {
        if (col.name === column.name) break;
        offset += parseInt(col.width || '0', 10);
      }
    }
    return offset;
  }
  
  // Calculează offset-ul pentru coloanele fixe la dreapta
  getRightOffset(column: ColumnDefinition): number {
    let offset = 0;
  ...
  // Returnează stilurile pentru coloanele fixe (sticky) în funcție de proprietatea 'fixed'
  getFixedStyle(column: ColumnDefinition): { [key: string]: any } {
  ...
  // Combină lățimea coloanei cu stilurile pentru fixare
  getCellStyle(column: ColumnDefinition): { [key: string]: any } {
    return {
